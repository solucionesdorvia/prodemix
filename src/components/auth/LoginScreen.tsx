...
const OAUTH_ERROR_MESSAGES: Record<string, string> = {
  OAuthSignin:
    "No se pudo iniciar sesión con Google. Revisá la configuración o probá de nuevo.",
  OAuthCallback: "Error al volver de Google. Probá de nuevo.",
  OAuthAccountNotLinked:
    "Ese email ya está asociado a otra forma de inicio de sesión.",
  AccessDenied: "Acceso denegado.",
  Configuration:
    "Falta configuración en el servidor (AUTH_SECRET, AUTH_URL o Google OAuth).",
  Default: "No se pudo completar el inicio de sesión con Google.",
};
...
type LoginScreenProps = {
  /** `true` si `GOOGLE_CLIENT_ID` y `GOOGLE_CLIENT_SECRET` están definidos en el servidor. */
  googleOAuthEnabled: boolean;
};

export function LoginScreen({ googleOAuthEnabled }: LoginScreenProps) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (res?.error) {
        setCredError(
          OAUTH_ERROR_MESSAGES[res.error] ?? OAUTH_ERROR_MESSAGES.Default,
        );
        return;
      }
      if (res?.url) {
      ...
      setCredError("No se pudo conectar con Google. Probá de nuevo.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="mx-auto flex min-h-dvh w-full max-w-lg flex-col px-4 pb-8 pt-[max(2rem,env(safe-area-inset-top))]">
      <header className={pageHeader}>
        ...
        <p className="mt-1.5 text-[12px] leading-relaxed text-app-muted">
          {googleOAuthEnabled ?
            "Creá tu cuenta con email y contraseña, o entrá con Google. Sesión segura en el servidor."
          : "Creá tu cuenta con email y contraseña. Sesión segura en el servidor."}
        </p>
      </header>
